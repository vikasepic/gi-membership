...
/**
 * Where Stripe sends somebody back after the card is saved.
 *
 * This is where the subscription is actually created — completeOfferCheckout is
...
export async function GET(request: Request) {
  ...
  if (!setupIntentId) return go("/library");
  ...
  try {
    result = await completeOfferCheckout(setupIntentId);
  } catch (e) {
  ...
  try {
    ...
    if (!user) {
      const tokenHash = await mintOfferLogin(setupIntentId, clientSecret);
